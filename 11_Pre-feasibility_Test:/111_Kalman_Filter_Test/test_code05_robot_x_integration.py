...
# 예측을 엔코더 값으로 적용했던것을 추정값으로 변경하고 엔코더 값이 들어오면 추정값을 보정하도록 변경함
# X방향 오차가 있어 적분 컨트롤 추가함
...
# ==========================================
# [클래스] 비주얼 서보잉 PI 제어기 (X축 PI 제어 반영 버전)
# ==========================================
class VisualServoingController:
    def __init__(self, dt=0.01):
        self.dt = dt
        self.kp = np.array([3.0, 3.5, 3.0])      # [X, Y, Z] 비례 게인
        
        # 적분 게인 및 누적 오차 변수 설정
        self.ki_x = 1.5                          # ★ X축 적분 게인 추가
        self.ki_y = 1.5
        self.integral_error_x = 0.0              # ★ X축 누적 오차 초기화
        self.integral_error_y = 0.0
        
        # 안티 와인드업(Anti-Windup) 제한값
        self.max_integral_x = 0.03                # ★ X축 최대 오차 누적 제한
        self.max_integral_y = 0.01
    ...
    def generate_control_command(self, estimated_target_pos, estimated_conveyor_vel, estimated_robot_pos):
        ...
        # --- ★ X축 제어 (PI + Feed-forward) ---
        self.integral_error_x += pos_error[0] * self.dt
        self.integral_error_x = np.clip(
            self.integral_error_x, -self.max_integral_x / self.ki_x, self.max_integral_x / self.ki_x
        )
        # 기존 P+FF에 적분항(self.ki_x * self.integral_error_x)을 추가하여 정상상태 오차 원천 제거
        cmd_vx = self.kp[0] * pos_error[0] + self.ki_x * self.integral_error_x + estimated_conveyor_vel[0]
